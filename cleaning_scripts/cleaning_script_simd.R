...
#read in data
simd <- 
  read_csv(here("data/raw_data/inpatient_and_daycase_by_nhs_board_of_treatment_and_simd.csv")) %>% 
  janitor::clean_names()
...
simd_clean <- simd_hb %>% 
  select(quarter, hb, simd, episodes, length_of_stay) %>% 
  drop_na(simd) %>%   
...
write_csv(simd_clean, here("data/clean_data/simd_clean.csv"))
